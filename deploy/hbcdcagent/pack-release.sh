...
emit_secret_units() {
  ...
  (
    ...
    # 智能预警加密票据 SSO（可选；两者都为空则功能关闭）
    HBCDCAGENT_SSO_CLIENT_ID="${HBCDCAGENT_SSO_CLIENT_ID:-}"
    HBCDCAGENT_SSO_KEY_B64="${HBCDCAGENT_SSO_KEY_B64:-}"
    HBCDCAGENT_SSO_KEY_VERSION="${HBCDCAGENT_SSO_KEY_VERSION:-1}"
    HBCDCAGENT_SSO_RATE_PER_MIN="${HBCDCAGENT_SSO_RATE_PER_MIN:-}"
    # 允许嵌入本工作台的父页面 origin（逗号分隔；留空即 frame-ancestors 'none'）
    HBCDCAGENT_BFF_FRAME_ANCESTORS="${HBCDCAGENT_BFF_FRAME_ANCESTORS:-}"
...
Environment=HBCDCAGENT_SSO_CLIENT_ID=${HBCDCAGENT_SSO_CLIENT_ID}
Environment=HBCDCAGENT_SSO_KEY_B64=${HBCDCAGENT_SSO_KEY_B64}
Environment=HBCDCAGENT_SSO_KEY_VERSION=${HBCDCAGENT_SSO_KEY_VERSION}
Environment=HBCDCAGENT_SSO_RATE_PER_MIN=${HBCDCAGENT_SSO_RATE_PER_MIN}
Environment=HBCDCAGENT_BFF_FRAME_ANCESTORS=${HBCDCAGENT_BFF_FRAME_ANCESTORS}
