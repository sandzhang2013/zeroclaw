...
export function AgentProvider({
  ...
  const [sessionId, setSessionId] = useState(() => {
    ...
    if (taskId) {
      return resolveTaskSessionId(agentAlias, taskId)
        ?? getOrCreateSessionId(agentAlias, userId);
    }
    if (userId) return getOrCreateSessionId(agentAlias, userId);
    return getActiveSessionId(agentAlias);
  });
    ...
    switch (msg.type) {
      ...
      case 'error':
        if (msg.code === 'SESSION_FORBIDDEN') {
          ...
          setActiveSessionId(agentAlias, nextId);
